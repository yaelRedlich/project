...
namespace BlIimplemetation
{
    internal class OrderImplementaion : BlApi.Iorder
    {
        ...
        void Iorder.DoOrder(Order order)
        {
            try
            {
                foreach (var item in order.listProduct)
                {
                    ...
                    int quantity = p._quantityInStock;
                    DO.Product p1 = p with { _quantityInStock = quantity - item._quantity };
        ...
        public void SearchSaleForProduct(ProductInOrder product, bool custumerFavorie)
        {
            product._saleList = _dal.Sale
                .ReadAll(s => s._productId == product._productInOrderId
                           ...
                           && (custumerFavorie || s._isForClubMembersOnly))
        ...
        public List<BO.SaleInProduct> AddProductToOrder(BO.Order order, int productId, int quantity)
        {
            ...
            {
                if (productInOrder._quantity + quantity > p._quantityInStock)
                {
                    throw new BlInvalidQuantityException("Invalid Quantity");
                }
            ...
            {
                if (quantity > p._quantityInStock)
                    throw new BlOutOfStockException("Invalid Quantity");
                ProductInOrder productInOrder1 = new ProductInOrder(productId, p._nameProduct, p._price, quantity, new List<SaleInProduct>(), 0);
